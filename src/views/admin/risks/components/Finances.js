...
const Finances = ({
  ...
  const calculateTotals = (data) => {
    ...
    for (let i = 0; i < totals.length; i++) {
      if (
        ...
        totals[i] =
          actualLoss[i] -
          (actualRecovery[i] + recoveryExpenses[i] + insuranceRecovery[i]);
        ...
        totals[i] =
          actualLoss[i] -
          (insuranceRecovery[i] + actualRecovery + recoveryExpenses);
      } else if (
        actualLoss[i] ||
        insuranceRecovery[i] ||
        actualRecovery ||
        ...
        totals[i] =
          actualLoss[i] -
          (insuranceRecovery[i] + actualRecovery + insuranceRecovery);
      } else if (
        actualLoss[i] ||
        insuranceRecovery[i] ||
        recoveryExpenses ||
        ...
        totals[i] =
          actualLoss[i] -
          (insuranceRecovery[i] + recoveryExpenses + insuranceRecovery);
      } else if (
        actualLoss[i] ||
        actualRecovery ||
        recoveryExpenses ||
        ...
        totals[i] =
          actualLoss[i] -
          (+actualRecovery + recoveryExpenses + insuranceRecovery);
      } else if (
        insuranceRecovery[i] ||
        actualRecovery ||
        recoveryExpenses ||
  ...
  const calculateRowTotal = (row) => {
    return row.values.reduce(
      (acc, value) => acc + (isNaN(value) ? 0 : parseFloat(value)),
      0
    );
  };
  ...
  const updateTotalCurrencies = (data) => {
    ...
    if (totalRow) {
      ...
      setTotalCurrenciesProps(total.toFixed(3));
    }
  };
    ...
    const updatedData = newData.map((row) => {
      if (row.name === "Total") {
        return {
          ...row,
          values: totals,
          total: calculateRowTotal({ ...row, values: totals }),
        };
      }
      return row;
    });
            ...
            <Tbody>
              {tableData.map((row, index) => (
                <Tr key={index} height="10px">
                  <Td fontSize={12} width="20%" height="10px">
                    {row.name}
                  </Td>
                  <Td height="10px">
                    <Text fontSize={12}>
              ...
              <Tr key={index} height="10px">
                ...
                <Td height="10px">
                  <Input
                    ...
                    fontSize={12}
                    value={
                      row.name === "Total" ? row.total : calculateRowTotal(row)
                    }
                    isReadOnly
                  />
                </Td>
