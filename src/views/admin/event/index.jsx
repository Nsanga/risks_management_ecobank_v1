...
import { updateEvent } from "reduxStore/events/action";
import { listEvents } from "reduxStore/events/action";
import { fetchEvent } from "reduxStore/events/action";
...
const Event = ({ profiles, entities }) => {
  ...
  const uploadPDF = async (pdfBlob) => {
    ...
    const response = await fetch(`${API_CONFIG.url}/api/v1/upload/files`, {
      method: "POST",
      body: formData,
    });
