...
import { updateEntityRiskControl } from "reduxStore/entityRiskControl/action";
import { useEffect, useState } from "react";
import { listEntityRiskControls } from "reduxStore/entityRiskControl/action";
...
function RiskForm({
  ...
  const handleAmend = async () => {
    const payload = {
      ...
      updates: {
        ...
        nomineeRisk: riskFormData.nomineeRisk
          ? riskFormData.nomineeRisk
          : undefined,
        reviewerRisk: riskFormData.reviewerRisk
          ? riskFormData.reviewerRisk
          : undefined,
        riskDescription: riskFormData.riskDescription
          ? riskFormData.riskDescription
          : undefined,
      },
    };
    // console.log(payload);
  ...
  useEffect(() => {
    if (selectedRisk) {
      ...
      handleChange({
        target: { name: "ownerEmail", value: selectedRisk?.ownerEmail },
      });
      handleChange({
        target: { name: "activeRisk", value: selectedRisk?.activeRisk },
      });
      const ownerOption = profilesOptions.find(
        (option) => option.value === selectedRisk?.ownerRisk
      );
      if (ownerOption) {
        ...
        setOwnerValue({
          value: selectedRisk?.ownerRisk,
          label: selectedRisk?.ownerRisk,
        });
      }
      const nomineeOption = profilesOptions.find(
        (option) => option.value === selectedRisk?.nomineeRisk
      );
      if (nomineeOption) {
        setNomineeValue(nomineeOption);
      } else {
        setNomineeValue({
          value: selectedRisk?.nomineeRisk,
          label: selectedRisk?.nomineeRisk,
        });
      }
      const reviewerOption = profilesOptions.find(
        (option) => option.value === selectedRisk?.reviewerRisk
      );
      if (reviewerOption) {
        setReviewerValue(reviewerOption);
      } else {
        setReviewerValue({
          value: selectedRisk?.reviewerRisk,
          label: selectedRisk?.reviewerRisk,
        });
      }
    }
  }, [selectedRisk]);
              ...
              <Box width="100%">
                <Select
                  ...
                  value={frequenciesOptions?.find(
                    (option) => option.value === riskFormData.frequency
                  )}
                  onChange={(selectedOption) =>
                    handleSelectChangeWithRemind("frequency", selectedOption)
                  } // Handle frequency change and set remindOn
                />
              </Box>
            </HStack>
          </FormControl>
