...
import { listEntityReports } from "reduxStore/reports/action";
import RiskAssessmentTable from "./components/RiskAssessmentTable";
import { listEventsReports } from "reduxStore/reports/action";
import { listEntityIncidentReports } from "reduxStore/incident-report/action";
...
const Reports = () => {
  ...
  const [openIncidentTrendsAnalysisTable, setIncidentTrendsAnalysisTable] =
    useState(false);
  const [openIncidentLossReportTable, setIncidentLossReportTable] =
    useState(false);
  ...
  } = useSelector((state) => state.IncidentReportReducer);
  ...
  const handleOpenView = () => {
    dispatch(
      listEntityReports({
        sesion: selectedData.session,
        targetEntityId: selectedData.entities,
        type: "riskControl",
      })
    );
  ...
  const handleOpenIncidentTrendsAnalysisTable = () => {
    ...
    dispatch(
      listEntityIncidentReports({ targetEntityId: selectedData?.entities })
    );
  };
  const handleOpenIncidentLossReportTable = () => {
    dispatch(
      listEventsReports({
        startDate: selectedEventData.start_date,
        endDate: selectedEventData.end_date,
        targetEntityId: selectedEventData.entities,
      })
    );
  ...
  const renderForm = () => {
    switch (selectedForm) {
      ...
      case "risk-register-report":
        return (
          <RiskRegisterForm
            handleOpenView={handleOpenRiskRegisterTable}
            onSelectionChange={handleSelectionChange}
            entities={entities}
            loading={loading}
          />
      ...
      case "risk-control-assessment":
        return (
          <RiskAndControlAssessmentDetails
            handleOpenView={handleOpenRiskAssessmentTable}
            onSelectionChange={handleSelectionChange}
            entities={entities}
            loading={loading}
          />
      ...
      case "risk-details-by-entity":
        return (
          <RiskDetailsForm
            handleOpenView={handleOpenRiskDetails}
            onSelectionChange={handleSelectionChange}
            entities={entities}
            loading={loading}
          />
      ...
      case "key-indicator-analysis":
        return (
          <KeyIndicatorAnalysisForm
            handleViewReport={handleOpenKIReport}
            onSelectionChange={handleSelectionChange}
            entities={entities}
            loading={loading}
          />
      ...
      case "incident-trends-analysis":
        return (
          <IncidentTrendsAnalysisForm
            handleOpenView={handleOpenIncidentTrendsAnalysisTable}
            onSelectionChange={handleSelectionChange}
            entities={entities}
            loading_incident_reports={loading_incident_reports}
          />
        );
      case "incident-loss-report":
        return (
          <IncidentLossReportForm
            handleOpenView={handleOpenIncidentLossReportTable}
            onSelectionChange={handleSelectionEventChange}
            entities={entities}
            loading={loading}
          />
        );
      case "incident-event-report":
        return (
          <IncidentEventReportForm
            handleOpenView={handleOpenEventRecoveriesTable}
            onSelectionChange={handleSelectionChange}
            entities={entities}
            loading={loading}
          />
      ...
      {openRiskRegisterTable && (
        <Box>
          <Flex
            alignItems="center"
            mb={4}
            onClick={() => setRiskRegisterTable(false)}
            cursor="pointer"
          >
            <ChevronLeftIcon />
            Back to Report's list
      ...
      {openRiskAssessmentTable && (
        <Box>
          <Flex
            alignItems="center"
            mb={4}
            onClick={() => setRiskAssessmentTable(false)}
            cursor="pointer"
          >
            <ChevronLeftIcon />
            Back to Report's list
      ...
      {openIncidentTrendsAnalysisTable && (
        <Box>
          <Flex
            alignItems="center"
            mb={4}
            onClick={() => setIncidentTrendsAnalysisTable(false)}
            cursor="pointer"
          >
            <ChevronLeftIcon />
            Back to Report's list
          </Flex>
          <IncidentTrendsAnalysisTable
            incident_reports={incident_reports}
            actual_incident_reports={actual_incident_reports}
            loading_incident_reports={loading_incident_reports}
          />
        </Box>
      )}
          ...
          <HStack spacing={6} align="stretch" flex="1">
            <Box w="350px" bg={cardBg} borderRadius="lg" p={4} shadow="md">
              ...
              <Accordion allowMultiple>
                    ...
                    <List spacing={2}>
                      <ListItem>
                        ...
                        <Button
                          ...
                          colorScheme="blue"
                          onClick={() =>
                            handleFormSelection("risk-register-report")
                          }
                          bg={
                        ...
                        <Button
                          ...
                          colorScheme="blue"
                          onClick={() =>
                            handleFormSelection("risk-control-assessment")
                          }
                          bg={
                ...
                <AccordionItem>
                  <AccordionButton
                    _expanded={{ bg: "teal.50", color: "teal.600" }}
                  >
                    <Box flex="1" textAlign="left" fontWeight="semibold">
                      System Reports
                    </Box>
                    ...
                    <List spacing={2}>
                      <ListItem>
                        <Button
                          ...
                          justifyContent="flex-start"
                          colorScheme="teal"
                          onClick={() => handleFormSelection("user-views")}
                          bg={
                ...
                <AccordionItem>
                  <AccordionButton
                    _expanded={{ bg: "cyan.50", color: "cyan.600" }}
                  >
                    <Box flex="1" textAlign="left" fontWeight="semibold">
